...
async def submit_talep(form: TalepFormBase):
    ...
    await db.talep_forms.insert_one(form_dict)
    return {k: v for k, v in form_dict.items() if k != "_id"}

@router.get("/talep", response_model=List[TalepForm])
async def get_talep_list(current_user: dict = Depends(get_current_user)):
    forms = await db.talep_forms.find({}, {"_id": 0}).sort("created_at", -1).to_list(100)
    return forms

@router.put("/talep/{form_id}/status")
async def update_talep_status(form_id: str, status: Dict, current_user: dict = Depends(get_current_user)):
    result = await db.talep_forms.update_one(
        {"id": form_id},
...
async def delete_talep(form_id: str, current_user: dict = Depends(get_current_user)):
    result = await db.talep_forms.delete_one({"id": form_id})
...
async def submit_ekspertiz(form: EkspertizFormBase):
    ...
    await db.ekspertiz_forms.insert_one(form_dict)
    return {k: v for k, v in form_dict.items() if k != "_id"}

@router.get("/ekspertiz", response_model=List[EkspertizForm])
async def get_ekspertiz_list(current_user: dict = Depends(get_current_user)):
    forms = await db.ekspertiz_forms.find({}, {"_id": 0}).sort("created_at", -1).to_list(100)
    return forms

@router.put("/ekspertiz/{form_id}/status")
async def update_ekspertiz_status(form_id: str, status: Dict, current_user: dict = Depends(get_current_user)):
    result = await db.ekspertiz_forms.update_one(
        {"id": form_id},
        {"$set": {"status": status.get("status"), "updated_at": datetime.utcnow()}}
    )
    if result.matched_count == 0:
...
async def delete_ekspertiz(form_id: str, current_user: dict = Depends(get_current_user)):
    result = await db.ekspertiz_forms.delete_one({"id": form_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Form not found")
    return {"message": "Form deleted"}
